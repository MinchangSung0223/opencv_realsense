...
def read_point_cloud(pcd_file, point_multiplier=1.0):
    ...
    cloud = o3d.io.read_point_cloud(pcd_file,remove_nan_points=False,remove_infinite_points=True)
    if cloud.is_empty():
        return False, None

    if point_multiplier is not None and point_multiplier != 1.0:
        cloud.points = o3d.utility.Vector3dVector(np.asarray(cloud.points) * point_multiplier)
...
if __name__ == '__main__':
    ...
    mesh_frame_est = o3d.geometry.TriangleMesh.create_coordinate_frame(size = 0.3, origin = [0,0,0]) #original camera frame
    mesh_frame_est.transform(T_cw)
    mesh_frame_est2 = o3d.geometry.TriangleMesh.create_coordinate_frame(size = 0.4, origin = [0,0,0]) #original camera frame
    
    filter_cloud_nan(cloud)  # filter nans so that cloud can be visualized in open3d

    o3d.visualization.draw_geometries([cloud, mesh_frame_est, mesh_frame_est2])
